					...
					int flags)
{
	...
	{
        /* 需要在offsetNumber位置上添加item */
        
		/* yes, check it */
		if ((flags & PAI_OVERWRITE) != 0)   
		{
            /* 直接覆盖offsetNumber位置上的记录 */
	...
	{
        /* offsetNumber 未传入, 进行自由分配位置(slot) */
    ...
    /*
     * 插入Item，根据needshuffle调整现有的item的位置
     */
 ...
 *	any reason.
 *  添加一个item到page中。
 * 返回值一个偏移量(offset),如果出于某种原因插入失败，返回一个偏移错误的代码(InvalidOffsetNumber)
 *
 ...
 * 将overwriter参数和is_heap参数设置为true，会设置PAI_OVERWRITE和PAI_IS_HEAP的标志位
 * overwrite=true=PAI_OVERWRITE
 * is_heap=true=PAI_IS_HEAP
 *
 *	!!! EREPORT(ERROR) IS DISALLOWED HERE !!!
 *
 */
